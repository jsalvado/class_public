...
int perturb_find_approximation_switches(
  ...
  /** - if there are switches, consider approximations one after each
    ...
    for (index_ap=0; index_ap<ppw->ap_size; index_ap++) {
	  ...
	  /* Check that there was no other approximation being switched at the same time.
	     If there is, advance the switching time by 1 per mil
	   */
	  for (index_switch_prev=0; index_switch_prev < index_switch_tot; index_switch_prev++)
	    if (fabs(unsorted_tau_switch[index_switch_prev] - mid) < precision)
	      mid *= 1.0001
  ...
  if (_scalars_) {
	...
	double T_F = pba->T_cmb*pba->lrs_T_F/(a/pba->a_today)*_k_B_/_eV_;//T in electronvolt
	  ...
	  + phi_M/(rho_lrs_bg/_eV4_to_rho_class) * g_over_M_mT * phi_M_prime*M_delta_phi; // Ivan
        ...
        dy[idx+1] = -(a_prime_over_a*(1.0-3.0*ca2_lrs) - phi_M/(rho_lrs_bg/_eV4_to_rho_class)/(1.+w_lrs)*(1.+ca2_lrs)*phi_M_prime)*y[idx+1] // Ivan
